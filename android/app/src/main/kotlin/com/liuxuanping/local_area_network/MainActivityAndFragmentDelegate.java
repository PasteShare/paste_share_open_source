...
public class MainActivityAndFragmentDelegate implements ExclusiveAppComponent<Activity> {
    private static final String TAG = "MainActivityAndFragmentDelegate";
    private static final String FRAMEWORK_RESTORATION_BUNDLE_KEY = "framework";
    private static final String PLUGINS_RESTORATION_BUNDLE_KEY = "plugins";

    // The FlutterActivity or FlutterFragment that is delegating most of its calls
    // to this MainActivityAndFragmentDelegate.
    @NonNull
    private MainActivityAndFragmentDelegate.Host host;
    @Nullable
    private FlutterEngine flutterEngine;
    @Nullable private MainSplashView flutterSplashView;
    ...
    MainActivityAndFragmentDelegate(@NonNull MainActivityAndFragmentDelegate.Host host) {
        this.host = host;
    }

    /**
     * Disconnects this {@code MainActivityAndFragmentDelegate} from its host {@code Activity} or
     * {@code Fragment}.
     *
     * <p>No further method invocations may occur on this {@code MainActivityAndFragmentDelegate}
     ...
     *       MainActivityAndFragmentDelegate.Host#configureFlutterEngine(FlutterEngine)}.
     * </ol>
     */
    ...
    public @NonNull Activity getAppComponent() {
        ...
        if (activity == null) {
            throw new AssertionError(
                    "MainActivityAndFragmentDelegate's getAppComponent should only "
     ...
     * {@link MainActivityAndFragmentDelegate.Host#provideFlutterEngine(Context)}.
     *
    ...
    View onCreateView(
        ...
        flutterSplashView = new MainSplashView(host.getContext());
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            flutterSplashView.setId(View.generateViewId());
        } else {
            // TODO(mattcarroll): Find a better solution to this ID. This is a random, static ID.
            // It might conflict with other Views, and it means that only a single MainSplashView
     ...
     *       MainActivityAndFragmentDelegate.Host#shouldDestroyEngineWithHost()} ()} returns true.
     * </ol>
     */
    void onDetach() {
        Log.v(TAG, "onDetach()");
    ...
    private void ensureAlive() {
        if (host == null) {
            throw new IllegalStateException(
                    "Cannot execute method on a destroyed MainActivityAndFragmentDelegate.");
        }
    }

    /**
     * The {@link FlutterActivity} or {@link FlutterFragment} that owns this {@code
     * MainActivityAndFragmentDelegate}.
     */
